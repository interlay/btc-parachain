...
pub mod pallet {
    ...
    pub enum Event<T: Config> {
        DepositStake(T::AccountId, T::SignedFixedPoint),
        DistributeReward(T::CurrencyId, T::SignedFixedPoint),
        WithdrawStake(T::AccountId, T::SignedFixedPoint),
    ...
    pub type Stake<T: Config> = StorageMap<_, Blake2_128Concat, T::AccountId, SignedFixedPoint<T>, ValueQuery>;
...
impl<T: Config> Pallet<T> {
    pub fn participants() -> Vec<(T::AccountId, T::SignedFixedPoint)> {
    ...
    pub fn deposit_stake(account_id: &T::AccountId, amount: SignedFixedPoint<T>) -> Result<(), DispatchError> {
    ...
    pub fn withdraw_stake(account_id: &T::AccountId, amount: SignedFixedPoint<T>) -> Result<(), DispatchError> {
