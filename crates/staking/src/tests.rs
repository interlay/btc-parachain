...
fn should_stake_and_earn_rewards() {
    run_test(|| {
        assert_ok!(Staking::deposit_stake(&VAULT, &ALICE, fixed!(50)));
        assert_ok!(Staking::deposit_stake(&VAULT, &BOB, fixed!(50)));
        assert_ok!(Staking::distribute_reward(KBTC, &VAULT, fixed!(100)));
        assert_ok!(Staking::compute_reward(KBTC, &VAULT, &ALICE), 50);
        assert_ok!(Staking::compute_reward(KBTC, &VAULT, &BOB), 50);
...
fn should_stake_and_distribute_and_withdraw() {
    run_test(|| {
        assert_ok!(Staking::deposit_stake(&VAULT, &ALICE, fixed!(10000)));
        assert_ok!(Staking::deposit_stake(&VAULT, &BOB, fixed!(10000)));

        assert_ok!(Staking::distribute_reward(KBTC, &VAULT, fixed!(1000)));
        assert_ok!(Staking::compute_reward(KBTC, &VAULT, &ALICE), 500);
        assert_ok!(Staking::compute_reward(KBTC, &VAULT, &BOB), 500);

        assert_ok!(Staking::slash_stake(&VAULT, fixed!(50)));
        assert_ok!(Staking::slash_stake(&VAULT, fixed!(50)));

        assert_ok!(Staking::deposit_stake(&VAULT, &ALICE, fixed!(1000)));
        assert_ok!(Staking::distribute_reward(KBTC, &VAULT, fixed!(1000)));

        assert_ok!(Staking::compute_reward(KBTC, &VAULT, &ALICE), 1023);
        assert_ok!(Staking::compute_reward(KBTC, &VAULT, &BOB), 976);

        assert_ok!(Staking::withdraw_stake(&VAULT, &ALICE, fixed!(10000)));
        assert_ok!(Staking::compute_stake(&VAULT, &ALICE), 950);

        assert_ok!(Staking::withdraw_stake(&VAULT, &ALICE, fixed!(950)));
        ...
        assert_ok!(Staking::compute_reward(KBTC, &VAULT, &ALICE), 1023);
        assert_ok!(Staking::compute_reward(KBTC, &VAULT, &BOB), 1975);
    })
}
...
fn should_stake_and_withdraw_rewards() {
    run_test(|| {
        assert_ok!(Staking::deposit_stake(&VAULT, &ALICE, fixed!(100)));
        assert_ok!(Staking::distribute_reward(KBTC, &VAULT, fixed!(100)));
        assert_ok!(Staking::compute_reward(KBTC, &VAULT, &ALICE), 100);
        assert_ok!(Staking::withdraw_reward(KBTC, &VAULT, &ALICE), 100);
        assert_ok!(Staking::compute_reward(KBTC, &VAULT, &ALICE), 0);
    })
}
...
fn should_not_withdraw_stake_if_balance_insufficient() {
    run_test(|| {
        ...
        assert_err!(
            Staking::withdraw_stake(&VAULT, &ALICE, fixed!(200)),
...
fn should_not_withdraw_stake_if_balance_insufficient_after_slashing() {
    run_test(|| {
        ...
        assert_ok!(Staking::compute_stake(&VAULT, &ALICE), 0);
        assert_err!(
            Staking::withdraw_stake(&VAULT, &ALICE, fixed!(100)),
            TestError::InsufficientFunds
        );
    })
}
...
fn should_force_refund() {
    run_test(|| {
        ...
        assert_ok!(Staking::deposit_stake(&VAULT, &VAULT, fixed!(100)));
        assert_ok!(Staking::deposit_stake(&VAULT, &ALICE, fixed!(100)));
        assert_ok!(Staking::slash_stake(&VAULT, fixed!(100)));
        assert_ok!(Staking::deposit_stake(&VAULT, &VAULT, fixed!(100)));
        assert_ok!(Staking::deposit_stake(&VAULT, &ALICE, fixed!(10)));
        assert_ok!(Staking::distribute_reward(KBTC, &VAULT, fixed!(100)));

        let vault_stake_pre_refund = Staking::compute_stake_at_index(nonce, &VAULT, &VAULT).unwrap();
        let vault_reward_pre_refund = Staking::compute_reward_at_index(nonce, KBTC, &VAULT, &VAULT).unwrap();
        let alice_stake_pre_refund = Staking::compute_stake_at_index(nonce, &VAULT, &ALICE).unwrap();
        let alice_reward_pre_refund = Staking::compute_reward_at_index(nonce, KBTC, &VAULT, &ALICE).unwrap();

        assert_ok!(Staking::force_refund(KBTC, &VAULT));

        nonce = Staking::nonce(&VAULT);
        let vault_stake_post_refund = Staking::compute_stake_at_index(nonce, &VAULT, &VAULT).unwrap();
        let vault_reward_post_refund = Staking::compute_reward_at_index(nonce - 1, KBTC, &VAULT, &VAULT).unwrap();
        let alice_stake_post_refund = Staking::compute_stake_at_index(nonce - 1, &VAULT, &ALICE).unwrap();
        let alice_reward_post_refund = Staking::compute_reward_at_index(nonce - 1, KBTC, &VAULT, &ALICE).unwrap();
...
fn should_compute_stake_after_adjustments() {
    ...
    run_test(|| {
        assert_ok!(Staking::deposit_stake(&VAULT, &VAULT, fixed!(100)));
        assert_ok!(Staking::deposit_stake(&VAULT, &VAULT, fixed!(1152923504604516976)));
        assert_ok!(Staking::slash_stake(&VAULT, fixed!(1152923504604516976 + 100)));

        assert_ok!(Staking::deposit_stake(&VAULT, &VAULT, fixed!(1_000_000)));

        assert_ok!(Staking::deposit_stake(&VAULT, &VAULT, fixed!(1152924504603286976)));
